/**
 ...
 @author Lea Verou, Lucy Poulton (small tweaks for dynamic language setting)
 */
(async function() {
    ...
    var _ = Prism.Live = class PrismLive {
        ...
        setLanguage(lang) {
            this.lang = lang;
            this.code.classList.forEach( x =>
                x.match(/lang(uage)?-.+/g ? this.code.classList.remove(x) : null));
...
// Static properties
    ...
    $.ready().then(() => {
        $$(":not(.prism-live) > textarea.prism-live, :not(.prism-live) > pre.prism-live").forEach(source => {
            if (!_.all.get(source)) {
                _.allInstances.push(new _(source));
            }
        });
    });
